...
def checkFolderThenCreate(imageSliceOutput,maskSliceOutput):
...
def plotOneImageInstance():
    ...
    img = nib.load(imgPath).get_fdata() # this return a numpy.memmap, which is memory efficient ndarray tyrpe of object. it works just like numpy.ndarray
    imgMin = np.min(img)
    imgMax = np.max(img)
    imgShape = img.shape
    imgType = type(img)
    ...
    idx = 50
    axes[0].imshow(img[:,:,idx], cmap='gray')
    axes[1].imshow(mask[:,:,idx], cmap='gray')
    # plt.show()
    return img, mask, imgPath, maskPath
...
# calling method to create output folder, if exist will recreate empty one.
checkFolderThenCreate(imageSliceOutput, maskSliceOutput)
...
img, mask, imgPath, maskPath = plotOneImageInstance()
